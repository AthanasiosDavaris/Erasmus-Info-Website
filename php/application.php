<?php
  ...
  $universities = [];
  $error_message = '';
  $has_application_period_start = false;
  $application_period_message = '';
  ...
  try {
    ...
    $settings_stmt = $pdo->query("SELECT start_date, end_date FROM application_settings WHERE id = 1");
    $settings = $settings_stmt->fetch();

    if ($settings && !empty($settings['start_date']) && !empty($settings['end_date'])) {
      $start_date = new DateTime($settings['start_date']);
      $end_date = new DateTime($settings['end_date']);
      $end_date->setTime(23, 59, 59); // Make sure the entire end day is included
      $current_date = new DateTime();

      if ($current_date >= $start_date && $current_date <= $end_date) {
        $has_application_period_start = true;
      } else {
        $application_period_message = "Η περίοδος υποβολής αιτήσεων είναι κλειστή. Οι αιτήσεις γίνονται δεκτές από " . $start_date->format('d/m/Y') . " έως " . $end_date->format('d/m/Y') . ".";
      }
    } else {
      $application_period_message = "Η περίοδος υποβολής αιτήσεων δεν έχει οριστεί ακόμα από τον διαχειριστή. Παρακαλώ ελέγξτε ξανά αργότερα.";
    }

    if ($has_application_period_start) {
  ...
  <body>
    ...
    <div class="container">
      ...
      <section class="application">
        <?php if ($has_application_period_start): ?>      
        ...
        <?php else: ?>
          <div class="period-message">
            <h2><?php echo htmlspecialchars($application_period_message); ?></h2>
            <p>Για οποιαδήποτε απορία, μπορείτε να επικοινωνήσετε με τους υπεύθυνους καθηγητές για το Erasmus.</p>
          </div>
        <?php endif; ?>
      </section>
    </div>
